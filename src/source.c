...
/*
 * For typed-record sources (record_id > 0), multi-reference field values are
 * stored as newline-separated position indices (integers) rather than slugs.
 * Before calling hyle_apply_view, resolve slug → position and pre-filter the
 * row set so that apply_view sees plain row IDs without the reference filter.
 *
