use crate::{Blueprint, Forma, FormaContext, Manifest, Outcome, Query, Row, Source};
...
pub fn display_value(
	...
	let blueprint: Blueprint = parse_json(blueprint_json, "blueprint")?;
	let outcome: Outcome = parse_json(outcome_json, "outcome")?;
	let value: crate::Value = parse_json(value_json, "value")?;
...
pub fn forma_to_query(
	...
	let id: Option<crate::Value> = {
		let v: crate::Value = parse_json(id_json, "id")?;
		if matches!(v, crate::Value::Null) { None } else { Some(v) }
	};
	let query = crate::forma_to_query(&forma, table_name, &context, id.as_ref());
	serde_json::to_string(&query).map_err(to_string)
}

/// resolve + is_single + columns in one call (server pre-filters/sorts/paginates).
/// Returns `{ outcome, rows, isSingle, columns }` JSON.
#[wasm_bindgen]
pub fn resolve_and_view(
	...
	struct Out {
		...
		rows: Vec<Row>,
		is_single: bool,
		columns: Vec<crate::view::Column>,
	}

	let blueprint: Blueprint = parse_json(blueprint_json, "blueprint")?;
	let manifest: Manifest = parse_json(manifest_json, "manifest")?;
	let source: Source = parse_json(source_json, "source")?;
	let outcome = blueprint.resolve(&manifest, &source).map_err(to_string)?;
	let rows = rows_from_outcome(&outcome);
	let is_single = crate::raw::is_single(&manifest, &outcome);
	let columns = derive_columns(&blueprint, &manifest).map_err(to_string)?;
	serde_json::to_string(&Out { outcome, rows, is_single, columns }).map_err(to_string)
}
